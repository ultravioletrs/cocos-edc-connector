...
* **Go** (for compiling `cocos-cli` and `encrypt.go`)
...
Run this command from your **buildroot** output directory (or the directory containing the guest kernel `./output/images/bzImage` and initrd `./output/images/rootfs.cpio.gz`):
```bash
qemu-system-x86_64 \
    -enable-kvm \
    -cpu EPYC-v4 \
...
Start the Java control plane runtime using JDK 17. Run this command from the root of the **cocos-edc-connector** repository directory (setting `-Dcocos.kbs.url` to the host LAN IP so the guest agent can connect back to the host KBS):
```bash
<path_to_jdk_17>/bin/java \
     ...
     -Dcocos.cli.path=<path_to_cocos>/build/cocos-cli \
